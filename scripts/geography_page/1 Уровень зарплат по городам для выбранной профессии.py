...
def create_html_table(yearly_count):
    ...
    if 'name' in yearly_count.columns:
        yearly_count = yearly_count.drop(columns=['name'])

        yearly_count.columns = ["Расположение", "Средняя зп выбранной вакансии"]  # Устанавливаем названия столбцов

    # Создаем HTML-таблицу с использованием Pandas
    html_string = yearly_count.to_html(
    ...
    with open(f'salary-by-city-my-vac.html', 'w', encoding='utf-8') as f:
        f.write(html_string)
...
if __name__ == "__main__":
    ...
    df_copy_salary_level = df_copy._to_pandas().pivot_table(index='area_name', values=['avg_salary', 'name'], aggfunc={'avg_salary':'mean', 'name':'count'}).reset_index().sort_values(by=["name",'avg_salary'], ascending=False).head(16)
    df_copy_salary_level[['area_name', 'avg_salary']].reset_index(drop=True)

    df_copy_salary_level = df_copy_salary_level.sort_values(by='avg_salary', ascending=True)
    create_html_table(df_copy_salary_level)
    ...
    plt.barh(df_copy_salary_level['area_name'], df_copy_salary_level['avg_salary'], color='blue')
    plt.title("Уровень зарплат java-программиста по городам, где доля вакансия больше 1 % всех вакансий", color='white')
    plt.xlabel('Средняя зарплата', color='white')
    plt.grid(axis='y', color='white')
    plt.savefig("Уровень зарплат java-программиста по городам, где доля вакансия больше 1 % всех вакансий.png", transparent=True, bbox_inches='tight')
