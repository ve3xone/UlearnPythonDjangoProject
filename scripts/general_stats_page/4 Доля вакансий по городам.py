...
def process_salary_data(df, table_curr):
    ...
    df_full_salary_level_all = df_copy._to_pandas().pivot_table(
        index='area_name', 
        values=['avg_salary', 'name'], 
        aggfunc={'avg_salary':'mean', 'name':'count'}
    ).reset_index().sort_values(by=["name",'avg_salary'], ascending=False)
    df_full_salary_level_all[['area_name', 'name']].reset_index()
    df_full_salary_level_head = df_full_salary_level_all[['area_name','name']].head(9)
    other_cities_count = df_full_salary_level_all['name'].iloc[9:].sum()
    other_cities_mean = df_full_salary_level_all['avg_salary'].mean()
...
# plt.gcf().set_facecolor('#F1F1F1')
    ...
    plt.savefig("Доля вакансий по городам.png", transparent=True, bbox_inches='tight')
    plt.close()
