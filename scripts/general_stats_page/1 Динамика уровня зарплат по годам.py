...
def create_html_table(yearly_salaries):
    # Преобразуем DataFrame Modin в Pandas только для вызова to_html
    if isinstance(yearly_salaries, pd.DataFrame):
        pandas_df = yearly_salaries._to_pandas()  # Конвертируем в Pandas DataFrame
    else:
        pandas_df = yearly_salaries  # Если это уже Pandas, используем его напрямую

    # Сброс индекса и корректное именование столбцов
    pandas_df = pandas_df.reset_index()  # Преобразуем индекс в столбец
    pandas_df.columns = ["Год", "Средняя зарплата"]  # Устанавливаем названия столбцов
...
def process_salary_data(df, table_curr):
    ...
    df_copy_salary_pivot = df_copy_salary.pivot_table(index='year', values=['avg_salary'])
    df_copy_salary_pivot = df_copy_salary_pivot.reset_index()
    df_copy_salary_pivot_save = df_copy_salary_pivot.pivot_table(index='year', values=['avg_salary'])
    ...
    plt.savefig("Динамика уровня зарплат по годам.png", transparent=True, bbox_inches='tight')
    plt.close()
